...
(function(c) {
	function p(d, a, b) {
		var e = this, l = d.add(this), h = d.find(b.tabs), j = a.jquery ? a : d
				.children(a), i;
		...
		else if (b.initialIndex === 0 || b.initialIndex > 0)
			e.click(b.initialIndex)
	}
	c.tools = c.tools || {
		version : "1.2.2"
	};
...
(function(d) {
	function r(g, a) {
		...
		var c = this, j = g.add(this), b = g.data("tabs"), h, l, m, n = false, o = p(
		...
		d.each("onBeforePlay,onPlay,onBeforePause,onPause".split(","),
				function(f, e) {
		...
		if (a.autoplay)
			m = setTimeout(c.play, a.interval);
		else
			c.stop();
		...
		if (!b.getConf().rotate) {
			...
			b.onBeforeClick(function(f, e) {
				if (e) {
					k.removeClass(i);
					e == b.getTabs().length - 1 ? o.addClass(i) : o
							.removeClass(i)
				} else
					k.addClass(i)
			})
...
(function(f) {
	...
	function t(a, b) {
		var c = this, h = a.add(c), e, i = 0, j = 0, m = a.attr("title"), q = n[b.effect], k, r = a
				.is(":input"), u = r
				&& a.is(":checkbox, :radio, select, :button"), s = a
...
(function(c) {
	...
	c.fn.autoscroll = function(d) {
		...
		this.each(function() {
			...
			b.autopause
					&& a.getRoot().add(a.getNaviButtons()).hover(function() {
						a.pause();
						clearInterval(i)
					}, function() {
	...
	d.fn.navigator = function(c) {
		...
		this
				.each(function() {
					...
					var e = d(this).data("scrollable"), k = p(e.getRoot(),
							c.navi), q = e.getNaviButtons(), l = c.activeClass, j = c.history
							&& d.fn.history;
					if (e)
						g = e;
					...
					e.onBeforeSeek(function(a, b) {
						var i = f().eq(b);
						!a.isDefaultPrevented() && i.length
								&& f().removeClass(l).eq(b).addClass(l)
					});
...
$(document)
		.ready(
				function() {
					...
					$('.productThumb ul').tabs('.productsWarp > ul > li', {
						effect : 'horizontal'
					}).slideshow({
						autoplay : true
					});
